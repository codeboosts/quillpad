import {
    GET_ALL_PRODUCTS_BY_JOB_LOGS_ID,
    GET_STAGING_FAILED_PRODUCTS,
  } from '../test/graphql/remediation-rules/queries';
  ...
  describe('User test', () => {
    ...
    it('should return failed-products with errors', () => {
      let req = request(server?.httpServer)
        .post(gql)
    ...
    it('sshould return staging-success-products or staging-failed-products by Job Logs Id', () => {
      ...
      return req.expect(200).expect((res) => {
        ...
        expect(res.body.data.getAllProductsByJobLogsId).toHaveProperty(
          'totalRecords',
        );
